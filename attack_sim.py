...
def simulate_port_scan(target_ip="127.0.0.1", count=20):
    ...
    for i in range(count):
        ...
        time.sleep(0.02)
    print(f"[OK] Sent {count} TCP SYN packets to ports 8000-{8000+count-1} rapidly.")
...
def simulate_ping_flood(target_ip="127.0.0.1", count=30):
    ...
    for i in range(count):
        ...
        send(pkt, verbose=False)
        time.sleep(0.01)
    print(f"[OK] Sent {count} ICMP Echo Request packets rapidly.")
...
def simulate_sqli(target_ip="127.0.0.1"):
    ...
    send(pkt, verbose=False)
    print(f"[OK] Sent HTTP GET request with malicious payload: 'UNION SELECT username, password'")
...
def simulate_ssh_bruteforce(target_ip="127.0.0.1", count=8):
    ...
    for i in range(count):
        ...
        send(pkt, verbose=False)
        time.sleep(0.05)
    print(f"[OK] Sent {count} SSH TCP connection attempts to port 22.")
...
def simulate_arp_spoofing(target_ip="127.0.0.1"):
    ...
    print(f"[OK] Sent ARP responses binding 192.168.1.50 to conflicting MAC addresses.")
...
def main():
    ...
    print(f"[TARGET] Target IP for attack simulation: {target}")
    print("[INFO] Make sure CodeAlpha NIDS dashboard (dashboard.py) is running to capture these alerts!\n")
    time.sleep(1)
    ...
    print("\n[COMPLETE] Attack simulation completed! Check the NIDS GUI dashboard for generated alerts.")
